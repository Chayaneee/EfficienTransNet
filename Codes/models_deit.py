...
class MViT(nn.Module):
    ...
    def forward(self, input):
        ...
        image_feature = image_embed.max(dim=1)[0]
...
class Classifier(nn.Module):
    def __init__(self, num_topics, num_states, vit=None, Xformer=None,
        ...
        self.txt_features = MultiheadAttention(embed_dim, num_heads, dropout) if Xformer != None else None
    ...
    def forward(self, img=None, txt=None, lbl=None, txt_embed=None, pad_mask=None, pad_id=3, threshold=0.5, get_embed=False, get_txt_att=False):
        ...
        if txt != None:
            if pad_id >= 0 and pad_mask == None:
                pad_mask = (txt == pad_id)
            txt_features = self.Xformer(token_index=txt, pad_mask=pad_mask) # (B,L,E)
        
        elif txt_embed != None:
            txt_features = self.Xformer(token_embed=txt_embed, pad_mask=pad_mask) # (B,L,E)
        ...
        if img != None and (txt != None or txt_embed != None):
            ...
            raise ValueError('vision and text must not be all none')
