...
def load_partial_data_from_json(input_file, num_elements):
...
class MIMIC(data.Dataset): # MIMIC-CXR Dataset
    def __init__(self, directory, input_size=(256,256), random_transform=True,
                ...
                max_len=1000, vocab_file='Vocabulary/mimic_unigram_1000.model'):
    ...
    def __get_reports_images(self, file_name='reports.json'):
        num_elements = 50000
        #caption_file = load_partial_data_from_json(self.dir + file_name, num_elements)
        ...
        for file_name, report in caption_file.items():
           ...
           # print(a)
            ...
            try:
                ...
                file_list = os.listdir(self.dir + 'files/' + pid + '/' + sid ) # + '/' +  '**-**-**-**-*.jpg'
               # print(file_list[0])
    ...
    def __get_nounphrase(self, top_k=100, file_name='/home/chayan/2.CGI/count_nounphrase.json'):
    ...
    def get_subsets(self, pvt=0.9, seed=0, generate_splits=True, debug_mode=False, train_phase=True):
        ...
        train_dataset.idx_pidsid = [(pid,sid) for pid,sid in train_files[train_indices]] if not debug_mode else [(pid,sid) for pid,sid in train_files[train_indices]]
        
        val_dataset = MIMIC(self.dir, self.input_size, False, 
                            self.view_pos, self.max_views, self.sources, self.targets, 
                            self.max_len, self.vocab_file)
        val_dataset.idx_pidsid = [(pid,sid) for pid,sid in train_files[val_indices]] if not debug_mode else [(pid,sid) for pid,sid in train_files[val_indices]]
...
class IUCXR(data.Dataset): # Open-I Dataset
    def __init__(self, directory, input_size=(256,256), random_transform=True,
                view_pos=['AP', 'PA', 'LATERAL'], max_views=2, sources=['image','history'], targets=['label'], 
                max_len=1000, vocab_file='Vocabulary/iucxr_unigram_1000.model'):
    ...
    def get_subsets(self, train_size=0.7, val_size=0.1, test_size=0.2, seed=0):
        ...
        train_dataset = IUCXR(self.dir, self.input_size, self.random_transform, 
